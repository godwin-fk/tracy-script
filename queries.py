...
def get_rtp_agentic_audit_logs_query(workflow_identifier,shipper_id, start_date, end_date):
...
def get_notifier_agentic_audit_logs_query(workflow_identifier,shipper_id, start_date, end_date):
    end_date = datetime.datetime.strptime(end_date, '%Y-%m-%d') + datetime.timedelta(days=1) # Adjusting for the 24hrs present in the end date we want to query
